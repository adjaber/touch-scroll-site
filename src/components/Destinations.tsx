...
const Destinations = () => {
  ...
  const destinations = [
    ...
    {
      name: language === 'ar' ? 'أمريكا الجنوبية' : 'South America',
          ...
          {destinations.map((destination, index) => (
            <Card key={index} className="overflow-hidden card-hover border-none">
              ...
              <CardContent className="p-4">
                ...
                <p className="text-gray-600 mb-1">{destination.countries} {t.countries}</p>
                <p className="text-travel-blue font-bold mb-3">{t.from} ${destination.startingPrice}</p>
